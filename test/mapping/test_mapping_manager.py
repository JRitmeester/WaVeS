...
from sessions.sessions import Session, Device, SoftwareSession, MasterSession, SystemSession
...
class MockSessionManager(SessionManagerProtocol):
    def __init__(self):
        ...
        self._master_session.mark_as_mapped = Mock(side_effect=lambda x: setattr(self._master_session, 'is_mapped', x))
        ...
        self._system_session.mark_as_mapped = Mock(side_effect=lambda x: setattr(self._system_session, 'is_mapped', x))
...
def session_manager():
    ...
    mock_session1.mark_as_mapped = Mock(side_effect=lambda x: setattr(mock_session1, 'is_mapped', x))
    ...
    mock_session2.mark_as_mapped = Mock(side_effect=lambda x: setattr(mock_session2, 'is_mapped', x))
...
def test_add_single_target_mapping(mapping_manager, session_manager):
    ...
    # Test master target
    mapping_manager._add_single_target_mapping("master", 0, session_dict, session_manager)
    assert session_manager.master_session in session_dict[0]
    assert session_manager.master_session.is_mapped is True
    
    # Test system target
    session_dict = {0: []}
    mapping_manager._add_single_target_mapping("system", 0, session_dict, session_manager)
    assert session_manager.system_session in session_dict[0]
    assert session_manager.system_session.is_mapped is True
    
    # Test device target
    session_dict = {0: []}
    mapping_manager._add_single_target_mapping("device:speakers", 0, session_dict, session_manager)
    assert any(isinstance(session, Device) for session in session_dict[0])
    
    # Test software session target
    session_dict = {0: []}
    mapping_manager._add_single_target_mapping("chrome.exe", 0, session_dict, session_manager)
...
def test_add_unmapped_sessions(mapping_manager, session_manager, config_manager):
    ...
    mapping_manager._add_unmapped_sessions(0, session_dict, session_manager, config_manager)
...
def test_add_unmapped_sessions_without_system(mapping_manager, session_manager, config_manager):
